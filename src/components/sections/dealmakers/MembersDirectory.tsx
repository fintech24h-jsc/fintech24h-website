...
export default function MembersDirectory({ locale = 'en' }: Props) {
  const copy = locale === 'ar' ? {
    ...
    searchAriaLabel: '搜索 DealMakers 成员',
    ofMembers: (shown: number, total: number) => `已显示 ${shown} / 共 ${total} 位成员`,
    noMatch: (q: string) => `没有与"${q}"匹配的成员。`,
    viewCard: '查看名片',
    viewCardAria: (name: string) => `查看 ${name} 的 DealMakers 名片`,
    loadMore: (n: number) => `加载更多（剩余 ${n} 位）`,
    close: '关闭',
    website: '网站',
    linkedin: 'LinkedIn',
    telegram: 'Telegram',
